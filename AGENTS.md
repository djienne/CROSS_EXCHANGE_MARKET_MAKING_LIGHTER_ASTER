...
- `run --mode live`, `taker run` without `--observe-only`, and the `*-market`/`*-roundtrip` probes submit real orders. Routine validation uses unit/local-transport tests and the dry run (`run --mode dry-run`, RUNBOOK "Dry run"), which needs no credentials and submits nothing real.
