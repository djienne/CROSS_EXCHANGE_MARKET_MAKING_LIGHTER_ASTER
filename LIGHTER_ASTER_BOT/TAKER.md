# Taker Engine (`lighter_aster_bot taker`)

The taker-taker engine runs from the same binary as the XEMM livebot, with its own
CLI (`lighter_aster_bot taker --help`) and config (`configs/taker-live-hype.toml`).
Any panic aborts the process, as the former standalone crate's `panic = "abort"` did.
...
Build and validate from `LIGHTER_ASTER_BOT/`. Repository policy requires release Cargo
...
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml status --market HYPE --json
...
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml run --markets HYPE --min-size --max-trades 1 --secs 300
```
...
```bash
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml aster-market-roundtrip --market HYPE --i-understand-live --max-usd 6
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml lighter-market-roundtrip --market HYPE --i-understand-live --max-usd 12
```
...
`configs/taker-live-hype.toml` is the parameter source. The entry gate records profitable,
...
```bash
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml resolve-session --market HYPE
./target/release/lighter_aster_bot taker --config configs/taker-live-hype.toml reset-circuit-breaker --market HYPE
```
