#!/usr/bin/env bash
# Deploy the bot image to the VPS WITHOUT compiling on the VPS (2 GiB RAM would
...
VPS_HOST="${VPS_HOST:-}"
KEY="${KEY:-}"
DEST="${DEST:-/home/ubuntu/LIGHTER_ASTER_BOT}"
IMAGE="${IMAGE:-lighter_aster_bot:live}"
...
sync_source() {
  ...
  tar czf - \
    Dockerfile docker-compose.yml .dockerignore config-live-lighter.toml configs \
    Cargo.toml Cargo.lock src scripts signers DOCKER_DEPLOY.md LIVE_RUNBOOK.md TAKER.md \
    | "${SSH[@]}" "tar xzf - -C '$DEST'"
