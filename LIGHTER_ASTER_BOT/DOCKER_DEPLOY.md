...
One image, `lighter_aster_bot:live`, runs both engines: the XEMM livebot (Aster post-only
maker orders, Lighter hedge) and the taker engine (`taker ...`, see [TAKER.md](TAKER.md)).
...
```bash
scripts/deploy_vps.sh source     # source + config + compose + signers -> /home/ubuntu/LIGHTER_ASTER_BOT
...
./target/release/lighter_aster_bot --config config-live-lighter.toml probe lighter-order-dry-run --market HYPE
./target/release/lighter_aster_bot --config config-live-lighter.toml verify-books --markets HYPE --secs 8
```
...
Do not delete counters while any signer process is running. Compose allows 200 seconds
for SIGINT draining (above the 175 s worst-case bounded drain) and has no automatic
restart policy.

## Docker Run
```bash
docker compose run --rm bot --config config-live-lighter.toml livebot --mode paper --markets HYPE --secs 30
...
```bash
./target/release/lighter_aster_bot --config config-live-lighter.toml \
