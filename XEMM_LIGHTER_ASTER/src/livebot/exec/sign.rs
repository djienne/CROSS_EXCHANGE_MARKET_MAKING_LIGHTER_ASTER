...
pub trait HlSigner: Send + Sync {
    ...
    fn sign_l1(&self, connection_id: &[u8; 32], is_mainnet: bool) -> Result<EvmSignature, SignError>;
}
...
impl EvmAsterSigner {
    ...
    pub fn new(user: String, signer: String, key: [u8; 32]) -> anyhow::Result<Self> {
        ...
        let derived = crypto::address_from_priv(&key)?;
        ...
        Ok(EvmAsterSigner { user, signer, user_bytes, signer_bytes, key })
    }
}
...
impl AsterSigner for EvmAsterSigner {
    ...
    fn sign_v3(&self, json_str: &str, nonce: i64) -> Result<Signature, SignError> {
        let sig = crypto::aster_sign_v3(&self.key, &self.user_bytes, &self.signer_bytes, json_str, nonce as u64)
...
impl HlSigner for EvmHlSigner {
    ...
    fn sign_l1(&self, connection_id: &[u8; 32], is_mainnet: bool) -> Result<EvmSignature, SignError> {
        crypto::hl_sign(&self.key, connection_id, is_mainnet).map_err(|e| SignError::Failed(e.to_string()))
    }
}
...
impl Default for AsterNonce {
    fn default() -> Self {
        AsterNonce { last_us: AtomicI64::new(0) }
    }
}
...
impl Default for MonotonicMs {
    fn default() -> Self {
        MonotonicMs { last_ms: AtomicI64::new(0) }
    }
}
    ...
    impl TestSigner {
        pub fn new() -> Self {
            ...
            TestSigner {
                ...
                hl: EvmHlSigner::new("0x062903894bce55d4f80ee5931c46c77cd7881351".into(), TEST_KEY).unwrap(),
            }
        }
    }
