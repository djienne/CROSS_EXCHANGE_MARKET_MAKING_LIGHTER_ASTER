...
//!
//! ABI validated against the bundled signer shared library:
...
impl Signer {
    ...
    pub fn load(
            ...
            ($name:literal, $ty:ty) => {{
                let s: Symbol<$ty> = unsafe {
                    lib.get($name).with_context(|| {
                        format!("missing symbol {}", String::from_utf8_lossy($name))
                    })?
                };
        ...
        let key = api_private_key
            .strip_prefix("0x")
            .unwrap_or(api_private_key);
    ...
    pub fn create_auth_token(&self, deadline_unix: i64, api_key_index: i32) -> Result<String> {
        let r =
            unsafe { (self.create_auth_token)(deadline_unix, api_key_index, self.account_index) };
