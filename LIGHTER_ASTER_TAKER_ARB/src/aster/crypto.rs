...
pub fn address_from_priv(key: &[u8; 32]) -> Result<[u8; 20]> {
    ...
    Ok(address_from_signing_key(&sk))
}

/// The Ethereum address of an already-parsed signing key. This avoids re-parsing key material on
/// live order hot paths that keep a [`SigningKey`] resident in memory.
...
pub fn sign_recoverable(key: &[u8; 32], prehash: &[u8; 32]) -> Result<([u8; 32], [u8; 32], u8)> {
    let sk = SigningKey::from_slice(key).map_err(|e| anyhow::anyhow!("bad private key: {e}"))?;
    sign_recoverable_with_key(&sk, prehash)
}

/// secp256k1 recoverable sign with an already parsed key.
pub fn sign_recoverable_with_key(
    sk: &SigningKey,
    prehash: &[u8; 32],
...
pub fn aster_sign_v3(
    ...
    let sk = SigningKey::from_slice(key).map_err(|e| anyhow::anyhow!("bad private key: {e}"))?;
    aster_sign_v3_with_key(&sk, user, signer, json, nonce)
}

/// Full Aster V3 signature using a cached secp256k1 key.
pub fn aster_sign_v3_with_key(
    key: &SigningKey,
    user: &[u8; 20],
    signer: &[u8; 20],
    json: &str,
    ...
    let (r, s, v) = sign_recoverable_with_key(key, &eip191)?;
...
mod tests {
    ...
    fn aster_golden_vector_byte_exact() {
        ...
        assert_eq!(
            hexstr(&digest),
            "0x356ed606bef291597ca9b9d03b9e7efaef1d30dd8621ec0c952e542dd3dd0ecd"
        );
    ...
    fn hl_msgpack_and_action_hash_match_oracle() {
        ...
        assert_eq!(
            hexstr(&hash),
            "0xdc76e4ee96d5ed33eae5fc25d0c3efd2ae108782b741627fd794be7b006eeba5"
        );
    ...
    fn vault_marker_changes_action_hash() {
        ...
        assert_ne!(
            no_vault, with_vault,
            "vault address must fold into the hash"
        );
